...
Page({
  ...
  clickButton:function(){
    if(this.data.players[this.data.current_player].canthrow
      ...
      &&!this.data.isGameOver){//只有骰子能被投掷过才能开始投掷，current_player==0说明当前是玩家回合
      this.startShake();
      this.playAnimation();
      var players=this.data.players;
  ...
  //因为人机对战永远是人机最后骰，所以判断是否超过3轮以及游戏是否结束都不需要在玩家的commit里判断
  commit: function() {
    //判断是否可以确定
    if((!this.data.players[this.data.current_player].canthrow//玩家不能投掷时可以确定
      ...
      if(current_player==1){//如果等于1，说明之后是人机回合，把人机掷骰子变量改为true
        setTimeout(()=>{
        this.setData({
          canAIthrow:true
        });
  ...
  addMul:function(){
    if(this.data.can_mul&&this.data.current_player==0&&!this.data.isGameOver){
      this.setData({
  ...
  AIAddMul:function(){
    ...
    var player_score=this.calculateBestLocks(player_points,player_locks)[1];

...
Page({
 ...
 wx.goEasy.connect({
     id:info.name,
     data:{"name":info.name,"avatar":info.avatarUrl},
  ...
  wx.goEasy.pubsub.subscribe({
    ...
    onMessage: function (message) {
        if(message.content.startsWith("OnLine"))
        {
            ...
            let name = match[1]
            let ava = match[2]
            players.push({id:name,unique:"",money:app.globalData.local_player_money,score:0,type:'',canthrow:true,avatarUrl:ava})
            
            if(Master == 1)
            {
                let str = JSON.stringify(players)
                if(cur_player == app.globalData.local_player_num)//人数已满，启动游戏
                {
...
wx.goEasy.pubsub.publish({
  ...
  startShake: function() {
    // wx.goEasy.pubsub.publish({
    //     channel: cur_channel,
    //     message: "startShake",//替换为您想要发送的消息内容
    //     onSuccess:function(){
    //         console.log("消息发布成功。");
    //     },
    //     onFailed: function (error) {
